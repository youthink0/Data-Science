...
def contrastive_loss_wo_cross_view(h1, h2, z):
    ###計算graph view中的contrastive_loss###
    #建立計算張量的lambda函式
    f = lambda x: torch.exp(x)
...
class MERIT(nn.Module):
    ...
    def forward(self, aug_adj_1, aug_adj_2, aug_feat_1, aug_feat_2, sparse):
        ...
        #分別計算各自的loss
        loss_network = self.beta * contrastive_loss_wo_cross_network(online_pred_one, online_pred_two, target_proj_two.detach())
        loss_view = (1.0 - self.beta) * contrastive_loss_wo_cross_view(online_pred_one, online_pred_two, target_proj_two.detach())
        l1 =  loss_network + loss_view
            
        loss_network = self.beta * contrastive_loss_wo_cross_network(online_pred_two, online_pred_one, target_proj_one.detach())
        loss_view = (1.0 - self.beta) * contrastive_loss_wo_cross_view(online_pred_two, online_pred_one, target_proj_one.detach())
        l2 = loss_network + loss_view
